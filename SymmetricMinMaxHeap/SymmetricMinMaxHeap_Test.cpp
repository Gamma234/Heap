...
void BuildHeapAndSort( Type* dataPtr, int size, bool smallToBig ) {
  ...
  SymmetricMinMaxHeap<Type>* smmh = new SymmetricMinMaxHeap<Type>() ;

  Type data ;

  for ( int i = 0 ; i < size ; i++ ) {
    smmh->Insert( dataPtr[i] ) ;
  } // for  

  if ( smallToBig ) {

    for ( int i = 0 ; smmh->GetMinAndDelete( data ) ; i++ ) {
      dataPtr[i] = data ;
    } // for
    ...
    for ( int i = 0 ; smmh->GetMaxAndDelete( data ) ; i++ ) {
      dataPtr[i] = data ;
    } // for
